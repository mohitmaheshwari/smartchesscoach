...
def test_event_serialization_is_deterministic_and_keeps_provenance():
    ...
    assert first["evidence"]["grade"] == "plan"
...
def test_diagnostic_only_event_cannot_enter_a_plan_even_if_detector_is_plan_grade():
...
def test_recurring_connection_requires_plan_grade():
    ...
    plan = _plan(shadow.event_id, role=ChapterRole.RECURRING_CONNECTION)
    with pytest.raises(ReviewContractViolation, match="lacks plan authorization"):
        plan.validate_against(event_index((shadow,)))


def test_prescribed_next_action_requires_plan_grade(monkeypatch):
    event = _event()
    monkeypatch.setattr(
        EventEvidence,
        "authorizes",
        lambda self, surface: surface == QualitySurface.CAPTION,
    )
