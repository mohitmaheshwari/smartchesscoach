...
def test_frozen_evidence_hashes_ignore_host_checkout_eol(tmp_path):
    lf_path = tmp_path / "evidence-lf.json"
    crlf_path = tmp_path / "evidence-crlf.json"
    lf_path.write_bytes(b'{"proof":true}\n{"value":3}\n')
    crlf_path.write_bytes(b'{"proof":true}\r\n{"value":3}\r\n')

    assert _file_sha256(lf_path) == _file_sha256(crlf_path)
