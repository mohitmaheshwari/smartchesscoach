...
class _Db:
    def __init__(self):
        ...
        self.positional_reason_generated_candidates = _Collection()
...
async def test_next_position_returns_structural_evidence_and_progress(fake_db):
    ...
    assert response["progress"] == {
        ...
        "by_disposition": {},
        "generated": 0,
        "generated_unique_candidates": 0,
        "verified_generated_captions": 0,
        "generated_by_disposition": {},
    }
...
@pytest.mark.asyncio
async def test_next_position_includes_generated_candidate(fake_db):
    from services.positional_reason_learning import position_fingerprint

    fingerprint = position_fingerprint(ROW["fen"], ROW["played_uci"], ROW["best_uci"])
    fake_db.positional_reason_generated_candidates.rows.append(
        {
            "position_fingerprint": fingerprint,
            "disposition": "eligible_positional",
            "concept_label": "Bring the king closer",
            "caption": "Rh1 leaves the king back. Kf3 brings it toward the centre.",
            "verification": {"status": "pass"},
        }
    )
    response = await route.next_position(user=None)
    assert response["generated_candidate"]["concept_label"] == "Bring the king closer"
    assert response["progress"]["verified_generated_captions"] == 1
