...
async def get_improvement_proof(user: User = Depends(get_current_user)):
    ...
    try:
        ...
        causal = {}
        # Most-recent-first: a gap can be prescribed more than once (trained,
        # completed, then re-prescribed after it recurred), and each
        # prescription independently qualifies once it has 3+ baseline and
        # current games. Without dedup that renders the same pattern as two
        # identical "proof" cards. Keep only the most recent evidence per gap.
        ...
        proof["training_causal"] = sorted(
            causal.values(), key=lambda c: -c["improvement_pct"])
