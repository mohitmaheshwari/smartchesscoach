...
async def get_coach_review(game_id: str, user: User = Depends(get_current_user)):
    ...
    raise HTTPException(
        status_code=410,
        detail=(
            "This legacy LLM review route is retired. Use the deterministic "
            "/api/games/{game_id}/coach-review route."
        ),
    )
...
async def get_opening_walkthrough(opening: str = None, user: User = Depends(get_current_user)):
    ...
    result = await generate_walkthrough(
        db,
        user.user_id,
        opening_name=opening,
        call_llm_func=None,
        include_stored_narrative=False,
    )
    return result
...
class MoveEvaluationRequest(BaseModel):
    """Server-owned request for a stored Game Review puzzle."""
...
class MoveEvaluationResponse(BaseModel):
    ...
    message: str
    comparison_to_original: Optional[str] = None
    feedback: str
...
async def evaluate_practice_move(
    request: MoveEvaluationRequest,
    user: User = Depends(get_current_user),
):
    """Grade Game Review practice from frozen stored evidence.

    The browser supplies only the canonical puzzle id and the move attempted.
    FEN, best move, accepted alternatives, original move, and detector verdict
    are all resolved server-side.  No Stockfish, LLM, or heuristic grading runs
    here.
    """
    puzzle_id = request.puzzle_id.strip()
    played_uci = request.user_move.strip().lower()
    if not puzzle_id or not played_uci:
        raise HTTPException(status_code=400, detail="puzzle_id and user_move are required")
    ...
    if not puzzle:
        raise HTTPException(status_code=404, detail="This review position is not ready for practice")
    ...
    if grade.get("recovery_credit_awarded"):
        try:
            ...
            logger.warning(
                "decay refresh after Game Review solve failed (non-fatal): %s",
                decay_err,
            )

    correct = bool(grade.get("correct"))
    quality = str(grade.get("quality") or "mistake")
    original_uci = str(
        puzzle.get("played_move_uci")
        or puzzle.get("user_move_uci")
        or puzzle.get("move_uci")
        or ""
    ).lower()
    comparison = None
    if original_uci:
        comparison = "same" if played_uci == original_uci else ("better" if correct else None)

    return MoveEvaluationResponse(
        quality=quality,
        symbol="check" if correct else "close",
        is_correct=correct,
        cp_loss=None,
        message={
            "best": "Perfect!",
            "excellent": "Excellent!",
            "mistake": "Try again",
        }.get(quality, "Try again"),
        comparison_to_original=comparison,
        feedback=str(grade.get("feedback") or "Review the position and try again."),
