...
async def get_coach_play_state(
    ...
    # 2026-07-03 (P1): Live mission scoreboard — computed from move_history each
...
async def predict_move_answer(request: Dict = Body(...), user: User = Depends(get_current_user)):
    """Record the guess; return hit/miss + the coach's actual move + the reveal text. Does NOT apply the
    move — the frontend then calls /trigger-coach-move, which applies the stored pending move (same one)."""
    global db
    from services.predict_coach_move import prediction_log_row

    session_id = request.get("session_id")
    guessed = request.get("guessed_move")
    if not session_id or guessed is None:
        raise HTTPException(status_code=400, detail="session_id and guessed_move are required")
    session_doc = await db.coach_sessions.find_one({"session_id": session_id})
    if not session_doc:
        raise HTTPException(status_code=404, detail="Session not found")
    if session_doc.get("user_id") != user.user_id:
        raise HTTPException(status_code=403, detail="Not your session")
    pending = session_doc.get("pending_prediction")
    if not pending:
...
async def get_geometry_plans_endpoint(
    ...
    """Compatibility endpoint for the retired always-on geometry overlay.

    Board Geometry now appears only after a learner completes the matching
    lesson, and its facts come from the canonical caption evidence pipeline.
    """
    return {"plans": [], "deprecated": True}
...
async def start_play_with_coach(
    ...
    # Coach Mode needs server resources (Stockfish, coaching), so limited to free users.
