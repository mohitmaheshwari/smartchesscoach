...
async def get_coach_home(user: User = Depends(get_current_user)):
    ...
    # ─── 2b. THE MIRROR (imported games always win when present) ───
    # Imported games are where the user's "in the wild" habits show up
    # — coach_sessions are practice with the coach literally present, a
    # different lens entirely. So whenever we have any analyzed imported
    # game, it takes the Evidence slot regardless of which is more recent.
    # Coach_session recap only survives when there are no imports at all.
    try:
