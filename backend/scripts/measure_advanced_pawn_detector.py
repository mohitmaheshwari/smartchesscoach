"""Measure `advanced_pawn_puzzle_proof` against Lichess's own theme labels.

Lichess's theme tags are the truth source: every `advancedPawn` puzzle the
...
Run it inside the backend container, which has the puzzle corpus:
    ...
    docker cp backend/scripts/measure_advanced_pawn_detector.py \
...
from services.advanced_pawn_puzzle_proof import (  # noqa: E402
    build_advanced_pawn_proof,
)
...
def run_batch(batch, show=0):
    ...
    for puzzle, (board, played, best, line) in batch:
        stats["n"] += 1
        bundle = build_advanced_pawn_proof(
        ...
        if bundle.verifier.verified:
            stats["verified"] += 1
            payoffs[bundle.verifier.facts[0]["payoff_kind"]] += 1
        else:
...
async def main():
    ...
    batch = await sample(db, "advancedPawn", args.recall)
    stats, payoffs = run_batch(batch, show=args.show)
    report("RECALL  advancedPawn", stats, payoffs)

    for theme in ("fork", "mateIn2"):
        batch = await sample(
            db, theme, args.crossfire, exclude=("advancedPawn",)
        )
        stats, payoffs = run_batch(batch, show=args.show)
        report(f"CROSS-FIRE  {theme} (excluding advancedPawn)", stats, payoffs)
