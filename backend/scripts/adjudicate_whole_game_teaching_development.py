...
TEACHING_INTELLIGENCE_PACKET_SCHEMA = (
    "deterministic_teaching_intelligence_evidence.v1"
)
TEACHING_INTELLIGENCE_MEMBERSHIP = (
    "e987cde54eb7df59aebf94ce710fef6991fcbb55787f5e2671956a286801a280"
)
TEACHING_INTELLIGENCE_WORKSHEET_SCHEMA = (
    "deterministic_teaching_intelligence_codex_worksheet.v1"
)
SUPPORTED_PACKETS = {
    EXPECTED_PACKET_SCHEMA: (EXPECTED_MEMBERSHIP, SCHEMA_VERSION),
    TEACHING_INTELLIGENCE_PACKET_SCHEMA: (
        TEACHING_INTELLIGENCE_MEMBERSHIP,
        TEACHING_INTELLIGENCE_WORKSHEET_SCHEMA,
    ),
...
def _line(
    ...
    if not replay.complete:
        raise ValueError("stored branch failed canonical legal replay")
    steps = []
    for event in replay.events:
        capture = (
            {
                "piece": event.captured_piece,
                "piece_id": event.captured_piece_id,
                "square": event.captured_square,
                "value_cp": event.captured_value_cp,
            }
        ...
        steps.append({
            "actor": (
                "mover" if event.actor == "initiator" else "opponent"
            ),
            "san": event.move_san,
            "uci": event.move_uci,
            "capture": capture,
            "gives_check": event.gave_check,
        })
    mate_for = None
    if replay.checkmate:
        mate_for = (
            "mover" if replay.checkmating_color == mover else "opponent"
        )
    return {
        "steps": steps,
        "material_delta_cp": replay.net_material_gain_cp,
        "settled_material_delta_cp": settled_material_gain_cp(replay),
        "mate_for": mate_for,
        "terminal": chess.Board(replay.final_fen).is_game_over(
            claim_draw=True
        ),
        "replay": replay,
    }
...
def _fork_targets(
    ...
    for square in board.attacks(move.to_square):
        ...
        targets.append({
            ...
            "piece_id": (
                f"{'white' if target.color == chess.WHITE else 'black'}:"
...
def _fork_payoff_proved(
    ...
    target_ids = {
        ...
        if target.get("piece") != "king"
    }
    return any(
        step["actor"] == "mover"
        and step.get("capture")
        and step["capture"].get("piece_id") in target_ids
...
def adjudicate_evidence(
    ...
    mate_info = evidence.get("mate_info") or {}
    mate_after = mate_info.get("after")
    mate_before = mate_info.get("before")
    if (
        played["mate_for"] == "opponent"
        and mate_after is not None
        and int(mate_after) != 0
    ):
        return {
    ...
    if (
        best["mate_for"] == "mover"
        and mate_before is not None
        and int(mate_before) != 0
    ):
        return {
            "evidence_verdict": "proved",
            "critical_false_claim": False,
    ...
    if played_reply and played_reply["capture"]:
        ...
        if (
            lost["value_cp"] - gained >= 100
            and played["settled_material_delta_cp"] is not None
            and played["settled_material_delta_cp"] <= -100
        ):
            return {
                "evidence_verdict": "proved",
                "critical_false_claim": False,
    ...
    if (
        missed
        and best["settled_material_delta_cp"] is not None
        and played["settled_material_delta_cp"] is not None
        and best["settled_material_delta_cp"] >= 100
        and (
            best["settled_material_delta_cp"]
            - played["settled_material_delta_cp"]
        ) >= 100
    ):
        return {
            "evidence_verdict": "proved",
    ...
    if (
        ...
        and _fork_payoff_proved(targets, best)
    ):
        return {
            "evidence_verdict": "proved",
            "critical_false_claim": False,
    ...
    played_move = parse_move(root, str(
        evidence.get("played_uci") or evidence["played_san"]
    ))
    ...
    if (
        ...
        played_promotion = (
            chess.piece_name(played_move.promotion)
            if played_move.promotion
            else None
        )
        best_promotion = chess.piece_name(best_move.promotion)
        promotion_choice = (
            played_promotion is not None
            and PIECE_VALUE_CP[best_move.promotion]
            > PIECE_VALUE_CP[played_move.promotion]
        )
        return {
            "evidence_verdict": "proved",
            "critical_false_claim": False,
            "cause_family": (
                "promotion_choice" if promotion_choice else "promotion_race"
            ),
            "causal_chess_fact": (
                (
                    f"{evidence['best_move_san']} made a {best_promotion}; "
                    f"{evidence['played_san']} made a {played_promotion}. "
                    "The stronger promotion kept more material in the "
                    "settled line."
                )
                if promotion_choice
                else (
                    f"{evidence['best_move_san']} promoted the pawn and kept "
                    "more material in the settled line."
                )
            ),
            "legal_settlement": [step["san"] for step in best["steps"]],
            "memory_cue": (
                "Before promoting, choose the piece; usually make a queen "
                "unless a tactic changes the answer."
            ),
        }
    return {
        ...
        "played_settled_material_delta_cp": (
            played["settled_material_delta_cp"]
        ),
        "best_settled_material_delta_cp": (
            best["settled_material_delta_cp"]
        ),
    }
...
def build_worksheet(packet: Mapping[str, Any], packet_sha256: str) -> Dict[str, Any]:
    packet_schema = str(packet.get("schema_version") or "")
    if packet_schema not in SUPPORTED_PACKETS:
        raise ValueError("unexpected packet schema")
    expected_membership, worksheet_schema = SUPPORTED_PACKETS[packet_schema]
    ...
    return {
        ...
        "source": {
            ...
            "stored_line_verifier_version": STORED_LINE_VERIFIER_VERSION,
            "games": 100,
        },
        "method": (
            "Detector-blind canonical legal replay and bounded forcing-move "
            "material settlement of complete games and stored branches. "
...
def main() -> None:
    ...
    rendered = json.dumps(worksheet, indent=2, sort_keys=True) + "\n"
