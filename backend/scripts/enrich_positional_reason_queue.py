...
def score_payload(
    ...
    *,
    ...
    pv_prefix: list[chess.Move] | None = None,
) -> dict[str, Any]:
    perspective = board.turn if pov_color is None else pov_color
    pov = info["score"].pov(perspective)
    wdl = info["score"].wdl(model="sf", ply=board.ply()).pov(perspective)
    pv = list(pv_prefix or []) + list(info.get("pv") or [])
    return {
...
def forced_line(
    ...
    """Analyse after the forced move so MultiPV cannot truncate its reply line."""
    mover = board.turn
    after = board.copy()
    after.push(move)
    if after.is_game_over():
        if after.is_checkmate():
            return {
                "score_cp": MATE_SCORE,
                "mate": 0,
                "wdl": {"wins": 1000, "draws": 0, "losses": 0},
                "first_uci": move.uci(),
                "pv_uci": [move.uci()],
                "pv_san": san_line(board, [move]),
                "depth": None,
                "seldepth": None,
                "nodes": 0,
            }
        return {
            "score_cp": 0,
            "mate": None,
            "wdl": {"wins": 0, "draws": 1000, "losses": 0},
            "first_uci": move.uci(),
            "pv_uci": [move.uci()],
            "pv_san": san_line(board, [move]),
            "depth": None,
    ...
    info = engine.analyse(after, chess.engine.Limit(nodes=nodes))
    return score_payload(board, info, pov_color=mover, pv_prefix=[move])
...
def enrich_row(engine: chess.engine.SimpleEngine, row: dict[str, Any], nodes: int) -> dict[str, Any]:
    ...
    forced_played = forced_line(engine, board, played, nodes)
    fresh_best = lines[0]["first_uci"] if lines else None
    forced_stored_best = forced_line(engine, board, best, nodes)
    ...
    enriched["engine_evidence"] = {
        ...
        "stored_best_line": forced_stored_best,
