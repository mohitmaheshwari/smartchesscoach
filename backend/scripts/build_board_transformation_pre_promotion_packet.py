"""Build the offline, blinded board-transformation pre-promotion packet."""
...
SCHEMA_VERSION = "board_transformation_causal.pre_promotion_review.v1"
GENERATED_ON = "2026-09-03"
DEFAULT_OUTPUT_PATH = BACKEND_ROOT / (
    "data/detector_gold/"
    "board_transformation_causal_pre_promotion_review_v1.json"
)


def _proof(row: Mapping[str, Any]):
    return build_board_transformation_opportunity_proof(
...
def build_packet() -> dict[str, Any]:
    ...
    public_cases.sort(key=lambda case: hashlib.sha256(
        f"board-transformation-review:{case['case_id']}".encode("utf-8")
    ).hexdigest())
    ...
    return {
        ...
        "database_reads": 0,
        "database_writes": 0,
        "quality_id": BOARD_TRANSFORMATION_CAUSAL_QUALITY_ID,
        ...
        "claim_under_review": (
            "The complete stored branches prove a multi-step board "
            "transformation, preserve each physical piece identity, and end "
            "in a positive legal material payoff after horizon resolution."
        ),
        "selection": {
            "candidate_policy": "all independent detector fires",
            "control_policy": (
        ...
        "review_rubric": {
            "verdict_values": [
                "proved_board_transformation_payoff",
                "not_proved",
                "insufficient_stored_horizon",
            ],
            "required_checks": [
                "both stored branches replay legally and completely",
                "every move between setup and payoff is retained",
                "physical pieces remain identical across all steps",
                "the claimed capture route is legal after the board changes",
                "post-horizon legal exchanges leave a positive net payoff",
                "the same payoff is absent from the played branch",
            ],
            "response_contract": {
