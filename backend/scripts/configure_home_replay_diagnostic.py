"""Safely enroll exact accounts in the Home replay diagnostic validation.

Credentials come only from the running container environment. The command is
read-only unless both --apply and the exact confirmation phrase are supplied.
"""
...
CONFIRMATION = "home-replay-validation"
FEATURE_PATH = "feature_flags.home_replay_diagnostic"
...
def feature_update(*, enabled: bool) -> Dict[str, Any]:
    return {
        "$set": {
            ...
            f"{FEATURE_PATH}.cohort": "home_replay_validation_2026_09",
        }
    }
