...
def main() -> int:
    ...
    fire_ids = set(fires)
    canonical_target_ids = forcing_family & set(target_fires)
    expected_new_ids = forcing_family - canonical_target_ids
    ...
    result = {
        "schema_version": (
            "hidden_opportunities_forcing_tempo_validation.v3"
        ),
        "proof_version": FORCING_TEMPO_CAUSAL_PROOF_VERSION,
        "canonical_target_line_proof_version": (
            TARGET_LINE_CAUSAL_PROOF_VERSION
        ),
        "ownership_policy": (
            "evaluate the current canonical target-line proof first; forcing "
            "tempo owns every remaining locked forcing-family position"
        ),
        "fresh_engine_runs": 0,
        "production_reads": 0,
        "database_writes": 0,
        ...
        "canonical_target_line_owned": len(canonical_target_ids),
