"""l4_pilot_monitor.py — health + progress monitor for the Universal Habit Coach 48h pilot.

Run anytime (t=0 sanity, interim, and the 48h read):
    docker exec -i chess-coach-backend python /app/backend/scripts/l4_pilot_monitor.py

Reports, per the pilot success criteria:
  - heartbeat() + silent_failure alarm
  - per-pilot-user: arm, focus integrity (is it the universal threat_scan focus on the right arm?),
    games played since assignment, clean rate, R_post-so-far, graduations
  - anomalies: focus corruption, unexpected zeros
The 48h verdict = measurement still accumulating, no unexpected zeros, no corruption, no alarms.
"""
...
async def main():
    ...
    print("\n=== PILOT COHORT (db.l4_pilot, cohort=pilot_48h) ===")
    print("user            arm        focus_ok  R_base   games_post  R_post   graduated")
    anomalies = []
    async for d in db.l4_pilot.find({"cohort": "pilot_48h"}, {"_id": 0}).sort("arm", 1):
        ...
        print(f"  {uid[:14]:15} {arm:10} {'yes' if focus_ok else 'NO!':8} "
              f"{str(d.get('r_base')):>7}  {ngames:>9}   {str(rpost):>7}   {graduated}")
