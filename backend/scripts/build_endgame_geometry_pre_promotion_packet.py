"""Build the offline, blinded endgame-geometry pre-promotion packet."""
...
SCHEMA_VERSION = "endgame_geometry_causal.pre_promotion_review.v1"
GENERATED_ON = "2026-09-03"
DEFAULT_OUTPUT_PATH = BACKEND_ROOT / (
    "data/detector_gold/endgame_geometry_causal_pre_promotion_review_v1.json"
)


def _proof(row: Mapping[str, Any]):
    return build_endgame_geometry_opportunity_proof(
...
def build_packet() -> dict[str, Any]:
    ...
    public_cases.sort(key=lambda case: hashlib.sha256(
        f"endgame-geometry-review:{case['case_id']}".encode("utf-8")
    ).hexdigest())
    ...
    return {
        ...
        "database_reads": 0,
        "database_writes": 0,
        "quality_id": ENDGAME_GEOMETRY_CAUSAL_QUALITY_ID,
        ...
        "claim_under_review": (
            "The stored branches prove one exact king-route, promotion-timing, "
            "or rook-geometry resource without asserting an unproved WDL."
        ),
        "selection": {
        ...
        "review_rubric": {
            "verdict_values": [
                "proved_endgame_geometry_resource",
                "not_proved",
            ...
            "required_checks": [
                ...
                "promotion identity and material gain are exact when claimed",
                "an exchange resource is not mislabeled as a material win",
                "no exact WDL or named technique is inferred from Stockfish PV",
