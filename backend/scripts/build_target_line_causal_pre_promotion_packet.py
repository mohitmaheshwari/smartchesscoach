...
# These frozen artifacts were generated before the repository declared an EOL
# policy. Some recorded SHA-256 values therefore bind to CRLF bytes and others
# to LF bytes. Reconstruct the recorded byte form in memory so provenance is
# reproducible on every checkout without rewriting historical evidence.
_FROZEN_CRLF_EVIDENCE_PATHS = frozenset({
    ...
    "backend/data/detector_gold/target_line_causal_independent_review_v3.json",
    "backend/data/detector_gold/target_line_causal_pre_promotion_answer_key_v3.json",
...
def _file_sha256(path: Path) -> str:
    ...
    try:
        ...
        relative = ""
    if relative in _FROZEN_CRLF_EVIDENCE_PATHS:
        raw = raw.replace(b"\n", b"\r\n")
