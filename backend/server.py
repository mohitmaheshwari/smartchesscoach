...
async def record_trap_attempt_endpoint(request: Request, data: dict, user: User = Depends(get_current_user)):
    ...
    Body:
    - trap_key: str - The trap identifier
...
async def get_user_trap_stats_endpoint(request: Request):
    ...
    Returns:
    - total_attempts, total_successes, success_rate
    - Per-trap statistics
...
async def get_trap_recommendations_endpoint(request: Request, limit: int = 5):
    ...
    Recommends traps based on:
    - Traps the user struggles with (low success rate)
    - Traps the user hasn't tried yet
...
async def get_global_trap_stats_endpoint(request: Request):
    ...
    - Most attempted traps
    - Hardest/easiest traps by success rate
    """
...
async def get_trap_leaderboard_endpoint(request: Request, trap_key: str, mode: str = "execution"):
    ...
    Shows top performers with at least 3 attempts.
    """
