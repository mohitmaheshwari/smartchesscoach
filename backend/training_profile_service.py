...
# =============================================================================
# DETERMINISTIC CHESS ANALYSIS LAYER
# =============================================================================
# This layer processes Stockfish output and makes ALL chess-related decisions.
# GPT should NEVER make chess decisions - only narrate facts from this layer.

class DeterministicChessAnalyzer:
    """
    Processes Stockfish output into structured chess facts.
    All chess logic lives here - GPT only gets facts to narrate.
    """
    ...
    MATE_EVAL = 9000  # Eval above this means mate
    WINNING_THRESHOLD = 300  # +3 pawns = winning
    ADVANTAGE_THRESHOLD = 100  # +1 pawn = advantage
    BLUNDER_THRESHOLD = 200  # Losing 2+ pawns = blunder
    MISTAKE_THRESHOLD = 100  # Losing 1+ pawn = mistake
    INACCURACY_THRESHOLD = 50  # Losing 0.5+ pawn = inaccuracy
    
    @staticmethod
    def classify_eval(eval_cp: int, is_mate: bool = False, mate_in: int = None) -> Dict:
        """
        Classify a position evaluation into human-readable assessment.
        Returns deterministic facts about who is better and by how much.
        """
        result = {
            "raw_eval": eval_cp,
            "is_mate": is_mate,
        ...
        if is_mate:
            if mate_in is not None:
                if mate_in > 0:
                    result["assessment"] = f"White has forced checkmate in {mate_in}"
                    result["winning_side"] = "white"
                    result["is_decisive"] = True
                else:
                    result["assessment"] = f"Black has forced checkmate in {abs(mate_in)}"
                    result["winning_side"] = "black"
                    result["is_decisive"] = True
            return result
        
        abs_eval = abs(eval_cp)
        if eval_cp > DeterministicChessAnalyzer.WINNING_THRESHOLD:
            result["assessment"] = "White is winning"
            result["winning_side"] = "white"
            result["advantage_pawns"] = eval_cp / 100
            result["is_decisive"] = True
        elif eval_cp > DeterministicChessAnalyzer.ADVANTAGE_THRESHOLD:
            result["assessment"] = "White has an advantage"
            result["winning_side"] = "white"
            result["advantage_pawns"] = eval_cp / 100
            result["is_decisive"] = False
        elif eval_cp < -DeterministicChessAnalyzer.WINNING_THRESHOLD:
            result["assessment"] = "Black is winning"
            result["winning_side"] = "black"
            result["advantage_pawns"] = abs(eval_cp) / 100
            result["is_decisive"] = True
        elif eval_cp < -DeterministicChessAnalyzer.ADVANTAGE_THRESHOLD:
            result["assessment"] = "Black has an advantage"
            result["winning_side"] = "black"
            result["advantage_pawns"] = abs(eval_cp) / 100
            result["is_decisive"] = False
        else:
            result["assessment"] = "Position is roughly equal"
            result["winning_side"] = None
            result["advantage_pawns"] = abs(eval_cp) / 100
            result["is_decisive"] = False
    ...
    @staticmethod
    def classify_mistake(cp_loss: int) -> Dict:
        """
        Classify a move based on centipawn loss.
        """
        result = {
            "cp_loss": cp_loss,
            "pawns_lost": cp_loss / 100,
        }
        
        if cp_loss >= 900:
            result["severity"] = "game_losing"
            result["label"] = "Game-losing blunder"
            result["emoji"] = "⛔"
        elif cp_loss >= DeterministicChessAnalyzer.BLUNDER_THRESHOLD:
            result["severity"] = "blunder"
            result["label"] = "Blunder"
            result["emoji"] = "🔴"
        elif cp_loss >= DeterministicChessAnalyzer.MISTAKE_THRESHOLD:
            result["severity"] = "mistake"
            result["label"] = "Mistake"
            result["emoji"] = "🟠"
        elif cp_loss >= DeterministicChessAnalyzer.INACCURACY_THRESHOLD:
            result["severity"] = "inaccuracy"
            result["label"] = "Inaccuracy"
            result["emoji"] = "🟡"
        else:
            result["severity"] = "minor"
            result["label"] = "Minor imprecision"
            result["emoji"] = "⚪"
        
        return result
    
    @staticmethod
    def detect_tactical_motifs(board: chess.Board, move: chess.Move = None) -> List[str]:
        """
        Detect tactical motifs in a position or after a move.
        ...
        for square in chess.SQUARES:
            ...
            if piece:
                ...
                if attackers and not defenders:
                    ...
                    motifs.append(f"undefended_{piece_name}")
                elif len(attackers) > len(defenders):
                    piece_name = chess.piece_name(piece.piece_type)
                    motifs.append(f"under_attack_{piece_name}")
        
        # Check for pins (piece blocking check to king)
        ...
        if king_square:
            for attacker_square in board.attackers(not board.turn, king_square):
                ...
                if attacker and attacker.piece_type in [chess.BISHOP, chess.ROOK, chess.QUEEN]:
                    ...
                    between = chess.SquareSet.between(attacker_square, king_square)
                    blockers = [s for s in between if board.piece_at(s)]
                    if len(blockers) == 1:
                        blocked_piece = board.piece_at(blockers[0])
                        if blocked_piece and blocked_piece.color == board.turn:
                            motifs.append("pin")
        
        return list(set(motifs))
    
    @staticmethod
    def analyze_move_type(board: chess.Board, move: chess.Move) -> Dict:
        """
        Analyze what type of move this is (capture, check, castle, etc.)
        """
        result = {
        ...
        if board.is_capture(move):
            ...
            if captured:
                result["captured_piece"] = chess.piece_name(captured.piece_type)
                result["capture_value"] = DeterministicChessAnalyzer.PIECE_VALUES.get(captured.piece_type, 0)
            elif board.is_en_passant(move):
                result["captured_piece"] = "pawn"
                result["capture_value"] = 100
                result["is_en_passant"] = True
        
        # Check if gives check
        test_board = board.copy()
        test_board.push(move)
        result["is_check"] = test_board.is_check()
        result["is_checkmate"] = test_board.is_checkmate()
        
        return result
    
    @staticmethod
    def categorize_mistake_type(
        board: chess.Board,
        ...
        """
        Categorize what TYPE of mistake this was.
        This is the key deterministic logic that decides mistake category.
        """
        result = {
            "category": "unknown",
            "subcategory": None,
        ...
        if stockfish_data.get("is_mate") and stockfish_data.get("mate_in", 0) > 0:
            result["category"] = "missed_checkmate"
            result["subcategory"] = f"mate_in_{stockfish_data['mate_in']}"
            result["description"] = f"Missed checkmate in {stockfish_data['mate_in']}"
            result["is_tactical"] = True
            return result
        
        # Analyze the best move
        best_move_info = DeterministicChessAnalyzer.analyze_move_type(board, best_move)
        played_move_info = DeterministicChessAnalyzer.analyze_move_type(board, move_played)
        
        # Missed winning capture
        if best_move_info["is_capture"] and not played_move_info["is_capture"]:
            capture_value = best_move_info.get("capture_value", 0)
            if capture_value >= 300:  # Bishop or higher
                result["category"] = "missed_winning_capture"
                result["subcategory"] = f"missed_{best_move_info['captured_piece']}_capture"
                result["description"] = f"Missed winning {best_move_info['captured_piece']}"
                result["is_tactical"] = True
                return result
        
        # Missed check/checkmate
        if best_move_info["is_checkmate"]:
            result["category"] = "missed_checkmate"
            result["description"] = "Missed immediate checkmate"
            result["is_tactical"] = True
            return result
        
        if best_move_info["is_check"] and not played_move_info["is_check"]:
            if cp_loss > 200:
                result["category"] = "missed_tactical_check"
                result["description"] = "Missed important check"
                result["is_tactical"] = True
                return result
        
        # Hung a piece (opponent can capture something after our move)
        opponent_threat = stockfish_data.get("opponent_threat")
        if opponent_threat and "x" in str(opponent_threat):
            result["category"] = "hung_piece"
            result["description"] = "Left a piece undefended"
            result["is_tactical"] = True
            return result
        
        # General tactical miss (high cp loss but no specific pattern)
        if cp_loss >= 300:
            result["category"] = "tactical_error"
            result["description"] = "Tactical mistake"
            result["is_tactical"] = True
            return result
        
        # Positional error (lower cp loss)
        if cp_loss >= 100:
            result["category"] = "positional_error"
            result["description"] = "Positional inaccuracy"
            result["is_positional"] = True
            return result
        
        result["category"] = "minor_inaccuracy"
        result["description"] = "Minor inaccuracy"
        result["is_positional"] = True
        return result
    ...
    def build_explanation_facts(
        ...
        """
        Build a complete set of FACTS for GPT to narrate.
        This is the main interface between chess analysis and GPT.
        
        GPT receives ONLY these facts - it cannot make any chess decisions.
        """
        facts = {
            "position_fen": fen,
            "move_played": move_played,
            "source": "stockfish_deterministic",  # Mark that this came from engine
        }
        
        # Position assessment
        eval_data = stockfish_data.get("eval", 0)
        is_mate = stockfish_data.get("is_mate", False)
        mate_in = stockfish_data.get("mate_in")
        
        eval_classification = DeterministicChessAnalyzer.classify_eval(eval_data, is_mate, mate_in)
        facts["position_before"] = eval_classification["assessment"]
        facts["is_decisive"] = eval_classification.get("is_decisive", False)
        
        # Best move information
        best_move = stockfish_data.get("best_move")
        best_line = stockfish_data.get("best_line", [])
        line_description = stockfish_data.get("best_line_description", "")
        
        facts["best_move"] = best_move
        facts["best_line"] = " → ".join(best_line) if best_line else ""
        facts["best_line_achieves"] = line_description
        
        # Checkmate detection
        if is_mate and mate_in and mate_in > 0:
            if mate_in == 1:
                facts["critical_fact"] = f"{best_move} is CHECKMATE"
                facts["is_checkmate"] = True
            else:
                facts["critical_fact"] = f"{best_move} forces checkmate in {mate_in} moves"
                facts["is_forced_mate"] = True
                facts["mate_in"] = mate_in
        elif "CHECKMATE" in line_description.upper():
            facts["critical_fact"] = f"{best_move} leads to checkmate"
            facts["is_checkmate"] = True
        
        # What happens after the played move
        if stockfish_data.get("after_played_is_mate"):
            mate_in_after = stockfish_data.get("after_played_mate_in", 0)
            if mate_in_after and mate_in_after < 0:
                facts["consequence"] = f"After {move_played}, opponent has forced checkmate in {abs(mate_in_after)}"
        elif stockfish_data.get("opponent_threat"):
            facts["consequence"] = f"After {move_played}, opponent threatens {stockfish_data['opponent_threat']}"
        
        # Material considerations
        if line_description:
            if "captures queen" in line_description.lower():
                facts["material_gain"] = "wins the queen"
            elif "captures rook" in line_description.lower():
                facts["material_gain"] = "wins a rook"
            elif "captures" in line_description.lower():
                facts["material_gain"] = "wins material"
