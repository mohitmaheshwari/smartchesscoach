...
class StockfishEngine:
    ...
    def get_candidate_lines(
        self,
        board: chess.Board,
        num: int = 8,
        depth: int = PUNISHMENT_DEPTH,
        pv_length: int = PUNISHMENT_PV_PLIES,
    ) -> List[Dict[str, Any]]:
        """Top `num` moves, each with its FULL continuation.

        get_top_replies() answers the same engine call but keeps only pv[0],
        which is enough to name a move and not enough to explain one. The
        on-demand "Why" button has to play each candidate out before it can
        tell whether that candidate explains itself, so it needs the line.

        Returns [{move_san, move_uci, eval_cp, line_san}] from the mover's
        point of view, best first. `line_san` starts WITH the candidate move.
        Empty list if the engine cannot answer.
        """
        if not self.engine:
            raise RuntimeError("Engine not started")
        try:
            ...
            for info in infos:
                ...
                if not pv:
                    continue
                # SAN has to be generated by walking a board; a detached
                # move list cannot be converted after the fact.
                walker = board.copy(stack=False)
                line_san: List[str] = []
                for mv in pv[:pv_length]:
                    try:
                        line_san.append(walker.san(mv))
                        walker.push(mv)
                ...
                cp = None
                if score is not None:
                    cp = score.pov(mover).score(mate_score=100000)
                out.append({
                    "move_san": line_san[0],
                    "move_uci": pv[0].uci(),
                    "eval_cp": cp,
            ...
            logger.error(f"Failed to get candidate lines: {e}")
            return []
