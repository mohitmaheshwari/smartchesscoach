...
async def generate_game_decryption_v5(
    ...
    try:
        ...
        for idx, move in enumerate(moves):
            ...
            # — matches the existing detector-firing gate. Hit events
            # (user played the pattern move correctly) require a future
            # detector-on-good-moves refactor and are not logged yet.
            if (is_user and best_move and best_move != move_san
                ...
                try:
                    ...
                    from services.pattern_event_logger import build_miss_event
                    _pattern_ids = resolve_pattern_ids(caption_facts)
                    for _pid in _pattern_ids:
                        pattern_miss_events.append(build_miss_event(
                            user_id=user_id,
                            game_id=game_id or "",
                            move_number=full_move_number,
                            move_san=move_san,
