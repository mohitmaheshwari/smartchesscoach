...
# v110 (2026-06-06): OPP-SIDE FAILURE-MODE PREDICATE FRAMEWORK (Phase 0 + 1).
# Permanent fix for "opponent's move is a mistake but doesn't say WHY" (fb_4899b11157fa
# Nbd7, fb_9f984e9753fc Nxf7 class). Phase 0: stockfish_service now STORES opponent
# best_move + PV for opp mistakes/blunders (previously computed then discarded), in a
# separate opponent_move_evaluations list; V5 merges it into eval_lookup so opp moves
# carry best_move + PV. Phase 1: caption_facts computes opp_failure_missed_capture +
# opp_failure_missed_mate; R12_blunder.json gains failure_mode_clauses_opp +
# opp_with_failure variants. Result: "Opponent's Nbd7 is a mistake — they had Qxd4,
# grabbing your pawn on d4 for free. Play O-O." auto-generates for ALL opp-missed-capture
# positions. Requires FULL RE-ANALYSIS (not just regen) for existing games to get opp
# data — new games get it automatically. traded_active_for_inactive + quiet_when_threatened
# variants filed for next session (CAPTION_BACKLOG).
# v109 (2026-06-06): PERMANENT FIX — authored_caption_overrides lookup was silently
# skipped on every regen because callers omitted the optional game_id parameter to
# generate_game_decryption_v5. coach.py and analysis_worker.py now pass game_id
# explicitly. Added a loud warning log when game_id is None so future regressions
# are caught immediately. Bumping forces regen so existing stored captions pick up
# all the overrides that have been written but never applied (including
# fb_4899b11157fa Nbd7 and any others sitting in authored_caption_overrides
# unconsumed).
# v108 (2026-06-06): force regen for the fb_4899b11157fa Nbd7 override. v107 bump
# happened BEFORE the override row was written, so regen produced the bare template
# and cached it. The override row was written AFTER the regen consumed v107. Need
# another bump to invalidate the cached render so the new override row gets picked
# up on next read. Architecture follow-up: the override layer should be checked on
# every read, not just during regen, so future override writes don't require a
# version bump. Filed for next session.
# v107 (2026-06-06): single manual override added for fb_4899b11157fa (Nbd7 m9 of game
# 1780f8bc) after engine verification (Stockfish depth 20 best_move=Qxd4). Engine-truth
# caption: 'Opponent's Nbd7 missed the free pawn on d4 — Qxd4 would have grabbed it cleanly.
# Play O-O — secure your king; the structure dodged a hit.' This is the same position
# where the existing fb_ec0098264c8e override flags 9.Qe2 as abandoning d4. Triggered the
# [[feedback_query_engine_before_authoring]] memory rule + the opp-side failure-mode
# predicate scope docs/opp_failure_mode_predicate_scope.md.
# v106 (2026-06-06): authoring-override batch applied — 22 new authored caption overrides
# went live (11 from tonight's Parth submissions: c6/f5/Rh4/c3/d6/Bf4/e5/Nxd5/cxd5/g5/Nxg3
# + 11 pre-existing pending submissions). 77 existing overrides refreshed. Bumping forces
# regen so stored decryption_v5_data picks up the new override layer on next read. Skipped
# fb_4d2363f0539b (Qe2) by manual decision — existing fb_ec0098264c8e override is more
# concrete (names Qxd4 + O-O explicitly).
# v105 (2026-06-06): two Parth fixes — (1) R12 recapture collision: when played + opp_reply
# both land on same square, switch to 'hangs your {piece} — opponent recaptures on {square}'
# instead of tautology 'Bxe5 hangs to Bxe5' (fb_22528b6266b1); (2) worst-placed-piece voice
...
V5_COACHING_VERSION = 178  # v178 (2026-09-26): the "why? play it out" line stops where the punishment lands. Mohit, on a one-move blunder that played out nine moves: "this straight up is one move blunder, our detector would have fired here and should have stopped directly on the first move, but didn't or that's not in place." Right that it should stop, wrong that anything was stopping it -- the line was [played] + pv_after_played[:8], a fixed slice with no stopping condition, so EVERY stored line came out exactly nine moves whatever the mistake was (19 of 19 measured, distribution {9: 19}). His card was Nf4 Bxf4+ Kb1 Bg4 h3 Bc8 Rhe1 Bb8 Nd4: the knight is lost at ply 1 and the other seven moves are the game continuing. punishment_line_end() now stops on the capture of the square the hang detector named (already carried as caption_facts.played_hangs_square), falls back to the last capture when no detector fired -- which preserves the case the slice was built for, eb189840 move 8 losing a pawn over seven forced captures -- and returns None rather than guessing when a positional punishment has neither, because cutting a line mid-idea is worse than showing it whole. Second bug found with it: of the three branches that build a line in the review, only pv_after_best applied coach_line_length_hint; the coach_line_moves branch, which is the one that renders on a user's OWN mistake, used the list raw. The cap was computed, stored as 9, and read by the branch that was not running. 84% of stored lines shorten on the fallback rule alone. Without this bump the stored reviews keep their nine-move lines. v177 (2026-09-25): the mistake floor stops telling a player they made the opponent's move. caption_fallback_tiers.tier23_caption hardcoded "You played X" and "taking your <piece>" in its flagged_mistake branch, with no mover_is_user guard -- _better_suffix immediately above it has one. So an OPPONENT mistake that fell to this floor rendered "You played Be7" under a card badged Opponent (flagged live on eb189840 move 12), and the possessive was wrong too: "Bh5 was stronger -- it moves your bishop out of danger" about a bishop the player does not own. Now uses the file's own _subject/_poss helpers. Measured over 3,000 games: 403 of 94,211 opponent cards (0.4%) said "You played", 77 of them also claiming "your" about opponent pieces. Small, but it is a false statement about the board, not just clumsy voice. v176 cards have already re-rendered at 176, so without this bump the fix reaches none of them. # v176 (2026-09-25): our OWN mistakes can now be played out on the board. build_move_teaching_decision only builds a coach line for OPPONENT mistakes, so a user mistake reached the card with coach_line_moves=None and coach_line_length_hint=None -- and the review UI's three ways of finding a line all fail on that: no trap, no coach line, and the pv_after_best branch needs the hint it never got. Every existing branch also animates pv_after_BEST, the line where you play WELL; nothing read pv_after_PLAYED, which is what answers "why was my move bad". Found on eb189840 move 8 (Nf3, -191cp): the loss is seven forced captures netting a pawn (Nxb5 Bxf6 Nxc3 Bxe7 Nxd1 Bxd8 Nxb2). No single piece hangs -- SEE is 0 on b5, c3 and g5 -- so _punishment_arrows correctly drew nothing, and no sentence carries seven even trades to a 1200 either. Played out on the board it is obvious in seconds. User mistakes >= 100cp now ship coach_line_moves = [played] + pv_after_played[:8], led by the played move so it animates from fen_before. Without this bump none of it reaches a stored review. # v175 (2026-09-23): a recommended reply no longer has to give CHECK to be drawn. Mohit on an inaccuracy card with a bare board: "did we not talk about inaccuracies too? Why is missing there?" The card read "Opponent's Qe7 is an inaccuracy. Play d5 -- your pawn kicks their knight on c6" and drew nothing, because the reply picture came from _check_attack_arrows, which abstains unless the move checks. d5 really does attack that knight -- SEE 200 on the board after it -- it simply is not a check, so the card named a move and left the player to find it. A check is what makes a threat unanswerable; it is not what makes it worth showing. The reply picture is now its own builder: the move first, then up to two targets SEE proves winnable, then the check when there is one. Measured over 500 games, 5,034 opponent cards with a usable reply: 261 (5.2%) drew before, 2,543 (50.5%) draw now, 77% of them with just two arrows and never more than four -- so this is nothing like the 14-arrow cage that made geometry read as clutter. All 2,543 verified against the board: 0 starting on an empty square, 0 naming a piece the reply does not winnably attack, 0 claiming a check that is not one. v174 (2026-09-23): the recommended reply is drawn as a MOVE, not as a line starting in mid-air. v172 pointed the opponent-card picture at the right move but _check_attack_arrows draws from the square the piece LANDS on -- which on that board is still empty, because the reply has not been played. Measured on the 92 reviews stored at v172: 45 of 77 opponent cards drawing arrows had every arrow starting on an empty square, so the only thing the picture visibly touched was the opponent piece it pointed at. That is the same complaint as before with the arrow reversed. The reply picture now leads with the move itself (d8->e7) so it begins on a piece the player can see and reads in order: move here, and it hits that. Silent when there is no attack picture to carry -- a bare move arrow on every opponent card is decoration, not a lesson. v173 (2026-09-23): a trapped piece is no longer waved off because one escape square is covered only by the KING. static_exchange_eval skips king recaptures by design (the v154 note says so in this file), so _recommended_move_traps_piece scored Bb2 at 0 and called the whole trap off -- on dfa9eed3 move 15 the black bishop on a3 had Bc5 200, Bb4 300, Bc1 300 and Bb2 0, and the card fell back to a generic 'look at what it attacks now'. The escape test now also asks legally_hanging_pieces, which knows about kings. Measured over 901 replayed games: 277 -> 335 fires (+58), 0 failing an independent re-check that every escape really loses material. Without this bump the stored reviews keep the old silence.   # v172 (2026-09-22): the arrows and the words now describe the same move, on the board the card is showing. Mohit: "Qe7 was black's move, but the arrwo shoed on opoponent queen" -- the card read "Opponent's b4 is a major blunder. Play Qe7 -- it attacks the queen on h7" and drew its arrow at White's queen. The review board renders fen_after (board_before + the PLAYED move) while _check_attack_arrows was computed on board_before + the BEST move, so the two agreed only when the player found the best move: over 500 games, 276 of 514 cards drawing that picture (54%) anchored it to a square that is EMPTY on the board being displayed. On an opponent card it was worse than misaligned -- best_move_uci there is the OPPONENT's best alternative, a move the caption never names, and "Qe7" was legal for both sides so two different moves wore the same three characters. The picture now comes from the move the card actually talks about: the played move on our own cards, and our best reply (resolved on the post-opponent board) on theirs. Measured over 5,036 real opponent moves, 159 -> 261 cards draw a picture and every one of the 14 where both paths fired drew DIFFERENT arrows. The other two teach-arrow builders -- the trap cage and the punishment arrows -- were computing on board_before + played_move all along, so this was the one source out of step. Found while fixing it: the dedupe kept the UNTAGGED copy of a colliding arrow and the suppression filter then stripped it, so Qd5+ forking the rook on a8 and the king on g8 rendered with a bare board -- the fork picture deleted by its own duplicate. The tagged copy now wins. The recommended-move picture Mohit asked for in fb_1c52480b2e9b is NOT dropped -- it was real teaching drawn over the wrong position, so it now ships on best_move_arrows with the FEN it is true of and the review page draws it under "What if I played X?", which already puts that exact board on screen. Net over 500 games: all 514 pictures kept (238 on the card, 276 relocated), 0 standing on an empty square. Without this bump none of it reaches a stored review. v171 (2026-09-20): the mate lessons reach a PLAYER, and the verifier stops silently deleting them. Mohit: "i don't see back rank mate captions fixed, have they?" They were -- in /admin/detector-review only. The rules had been written into that route's own caption producer, so a game review still read "Bxd5 allows mate in 3." and nothing else: two caption paths describing one position, which is what one-source-of-truth exists to stop. services/mate_lesson.py now owns the ORDER (named pattern > situations where a generic lesson would be unfair or false > the habit > today's symptom) and R01_mate.json owns the WORDS; the review page and the admin queue both ask it and neither writes prose. 195 lines of duplicated logic deleted from the route, admin distribution byte-identical afterwards (253 plan / 154 back-rank / 144 in-check / 128 fallback), 0 claims contradicted. Then the caption still did not ship: narrator_claim_verifier assumed any mate mention on a user move meant OUR best move mates, so it rejected even the known-good control "Bxd5 allows mate in 2." with "claims checkmate but c3 is not mate" and every allowed-mate caption was being replaced by the safe floor. Found by running a control caption through it. Fixed by reading the evidence for who is mating rather than guessing from prose, and by replaying the OPPONENT's line -- after the played move, which the first attempt forgot. That first attempt also failed OPEN on empty or unparseable lines: 6,632 captions passed while their line never reached mate. It now fails closed and proves every one: 360 pass, 0 false. v170 (2026-09-19): a missed discovered attack now says what the board shows. tactic:discovered_attack_with_stored_payoff is the first detector in the product promoted to caption grade on HUMAN semantic review -- 49 positions Mohit ruled one at a time with the board and both lines in front of him, 0 wrong, Wilson lower bound 92.7% -- instead of on two implementations agreeing, which is what put simple_hang at a false 96.9%. The grade had authorised a caption that nothing rendered: caption_pipeline and caption_facts never called the detector, so it reached users only as a puzzle. Detector #15 in inject_user_blunder_detector_facts now calls it and R12_blunder renders the geometry ("it steps your knight out of the way of your queen on f6, which is looking straight down that line at their rook on a1"). Gated on the live grade, not a flag, so dropping the grade to shadow silences it with no code change. The winnable-target threshold moved out of the review page and into the detector's own verifier so the queue, the caption path and verified_puzzle_builder cannot drift apart on it -- verified as a no-op, 0 of 56 rulings change served state. Fires on 49 of 52 ruled-true positions; the 3 it drops are ones the payoff verifier rejects because the stored line never collects the target. v169 (2026-09-18): a player getting checkmated is no longer told a piece is hanging. Mohit's rule -- a bigger loss means a PIECE went, not a pawn -- holds cleanly over 9,000 user mistakes (piece:pawn goes 1.1x, 1.5x, 2.2x, 4.0x, 8.1x as the loss grows) and then REVERSES above 900, where 47.9% of moves lose no material at all because they are mate swings. Testing it found 41 cards reading "your queen on e3 is hanging" when the truth was "you are mated in 6", including one where the player was ALREADY being mated before the move. Gating on cp_loss alone left 11 -- a player already lost walks into mate and the SWING is small (one real case at 3,449) while the position is mate. So the gate is the mate on the board, not the size of the subtraction: callers pass mate_in_play (the review queue has mate_info, the caption path has the evals). 41 -> 0, with all 24 genuine material hangs in that band untouched. v168 (2026-09-18): the caption path finally applies the hang detector's own gates. caption_facts called detect_played_hangs(board_before, played_move) with NO cp_loss, and the detector's docstring says that is raw/testing mode -- so the gates validated on 2026-06-06 against 105 flagged FENs never applied to anything a player read. R12_blunder fires from cp_loss>=30; the detector wants >=100. Measured over 1,238 hang facts on R12-eligible moves: 542 (44%) are dropped once the gates run. 332 sit at cp_loss 30-99, below the validated threshold; 220 name a piece too small to explain the loss, including 'O-O-O at cp_loss 676 names the pawn on a3' -- which is the exact case the docstring cites as the reason the gate was written. v167 (2026-09-18): a piece that just CAPTURED is trading, not hanging. played_hangs_detector feeds played_hangs_result into R12_blunder -- the caption that tells a player why their move was a mistake -- and it called a recapture a hang: Bxf3 takes a knight, gxf3 takes the bishop back, net zero, and the card read "it leaves your bishop on f3 hanging". 193 of 890 production fires (21.7%) were that shape. The move in that example IS a mistake, for an unrelated reason (Nxf3+ forks king and rook AND discovers onto an undefended a1 rook), so the cp_loss>=100 gate could never filter it. Two more defects found with it: board.attackers() is PSEUDO-legal so a PINNED attacker made a safe piece read as hanging (9 of 13 false claims in a 120-fire sample), and the cheapest-attacker heuristic ignored how many defenders stood behind the piece. Measured 11% -> 4% -> 0% provably false. Reviews below v166 already re-render on open; without this bump the 168 reviews stored AT v166 would keep serving the old caption. Found by Mohit on his second card in /admin/detector-review. v166 (2026-09-17): a recommended check now says what it WINS, and the board draws the two shapes that explain a mistake. Reported as fb_1c52480b2e9b: "Qa5+ was stronger - it gives check, forcing your opponent to respond" named the tempo and hid the point, which is that the queen hits the king down the a5-e1 diagonal AND a defended bishop along the fifth rank (+539 vs +109 for the move played). Two pictures ship with it: the check that adds a second attacker, and what a blunder gave away. Arrows were previously drawn on 0% of 5,039 user-mistake cards while appearing on 7.5% of all cards, which is why the geometry read as decoration; unfocused arrows are now suppressed (REVIEW_LEGACY_ARROWS restores them) and mistake-card coverage goes 0% -> 58.3%. Without this bump none of it reaches a stored review. v165 (2026-09-16): the trap picture now shows the PIECE'S OWN MOVES, not just our attackers. Drawing only d5->c6 etc said "these squares are covered" without ever showing they are the knight's squares - the L-shape the cue talks about was invisible. Three colours, three meanings: green = the move you played, yellow = every square the piece can still reach, red = your piece covering it. Yellow arrows are never dropped by the arrow cap because they ARE the lesson. v164 (2026-09-16): v164 (2026-09-16): the trap cage is drawn only on the card whose caption is about it. Suppressing the repeated cxb4 sentence left the six arrows still drawn under a caption about an even trade - board and words disagreeing. v163 (2026-09-16): v163 (2026-09-16): the trap lesson is said once. b4 traps the knight on a5; Black answers Bxb4 and cxb4 rebuilds the identical cage, so two consecutive cards rendered the same sentence and the same six arrows. If our previous move already trapped the same piece on the same square, the later card stays silent. v162 (2026-09-16): v162 (2026-09-16): a move that cages an enemy piece now says so. That card used to render "b4 is the best move in this position" - true, and worth nothing to a 600 - while the board was already drawing the whole trap. The caption is deliberately one short sentence naming the piece and its square, because caption_arrows carries the picture; the geometry lesson (a knight's L needs room on two sides, so the rim halves it) moves to principle_cue. Words only carry what the arrows cannot. v161 (2026-09-16): v161 (2026-09-16): a trapped piece is now SHOWN, on the move that traps it. The trapped-piece detector already played out every escape square and kept only the count; it now returns which of our pieces covers each one, plus the squares blocked by the victim's own men. That evidence becomes caption_arrows + highlight_squares on OUR move (previously the trap was only ever described in words on the OPPONENT's card, after the fact). Piece-agnostic: the detector skips only pawns and kings, so a trapped queen, rook or bishop draws the same cage. caption_arrows had been emitted on every card since the pipeline was written and read by nothing - the review UI now consumes it at all three move-selection sites. v160 (2026-09-16): v160 (2026-09-16): principle_cue - the amber "how to spot it next time" line the review card already renders - has been EMPTY on every card since v139 (measured 0% across v139..v159 on 3,605 mistake cards, vs 39% at v135). Cause: all 35 catalog principles are graded shadow(34)/disabled(1), so can_influence(.., CAPTION) is False for every one and the gated block can never set a cue. Captions became pure diagnosis - correct about what happened, silent about how to see it coming. This adds cues derived from facts the pipeline ALREADY board-verified (trapped piece, undefended recapture target, fork, opponent capture, free capture), phrased as a scan the player can run on an unseen position rather than a fact about the world. Asserts nothing new and does not touch the quality gate. v159 (2026-09-15): a move that IS the engine's top choice, and any move that delivers checkmate, can no longer be captioned as a fault. Measured on the 426 user-flagged moves carrying FEN+caption: 67 were the engine's own top move at depth 16 and ~15 of those were fault-framed ("Opponent's Rxd6 is an inaccuracy. Play Rxd6"; Qf8# captioned "Weak Squares"). The old downgrade lived inline in this file gated on is_user so opponent moves were never covered, and PWC never reached it at all - the guard now sits in BOTH caption_pipeline.compute_severity_for_move and build_move_teaching_decision, and downgrades practical severity too (it drives variant selection). Without this bump the fix would not reach existing users' stored decryption_v5_data. v158 (2026-09-14): two more why-families for opponent mistakes, and the hedging templates now check whether the player had a choice. fb_f6050ba76406 wanted the scan named -- an opponent mistake where taking back would cost an UNDEFENDED piece now says which piece has no defender. fb_ca8cc3ec9c5f shipped as a bare "Qb1+ is a major blunder"; when every legal answer to a check also attacks the checking piece, the caption says so. fb_fbdee5ee58be dismissed Qd1 as "does not do much" when it was played IN CHECK with three legal moves and was the engine's own choice; check, forced and best now suppress the hedge. # allow-noncentral-caption v157 (2026-09-14): a capture is only called free when nothing can take back. caption_facts measures on the board AFTER the move, so the recapturers sit in attackers_on_target while defenders_on_target holds our own pieces guarding the square we landed on -- _facts_caption read the defender list and called every guarded capture free. 198 of 414 "wins it for nothing" captions were false (47.8%); fb_6a7c9f8823ac was Bxf6 on a knight guarded by both the e7 bishop and the g7 pawn. Those now read "takes the knight, but their bishop and their pawn can take back -- so it is an even trade". # allow-noncentral-caption v156 (2026-09-13): caption word cap 45 -> 60, and a cut now logs itself. Re-rendering 150 games with the cap lifted (8,494 captions) put p50 at 12 words, p99 at 40, p99.5 at 44 and the max at 94 -- only 0.45% ran past 45, but they were concentrated in R12_blunder, the rule that explains why a move was a mistake. The cut takes the last sentence boundary, which leaves no ellipsis, so a coaching sentence could vanish with nothing to show it: a "Play b4 ..." recommendation did exactly that. # allow-noncentral-caption v155 (2026-09-13): an opponent mistake now says WHY it was a mistake, not just what to play. When the recommended reply leaves a piece with no safe square, the card names it and gives the geometry that transfers -- "their knight on a5 then has nowhere safe to go -- a knight on the edge has only four squares to jump to". 91% of 29,087 opponent mistake cards previously stated a verdict and a move and stopped. Also: the card after a recommendation no longer scolds the player for taking it (m11 said "Play b4", m12 answered "that can be okay, but first get your other pieces out"). # allow-noncentral-caption v154 (2026-09-11): a recommended capture is only called a trade when the exchange says so. _recommended_move_why described every recommended capture from the static exchange value on the target square and its last branch -- labelled "equal-ish exchange" -- had no floor under it, so a capture that loses a whole piece there was sold to the player as a trade. Flagged live on game 46edaf7e move 8: "Play Nxb5 -- it trades his pawn", where SEE on b5 is -200 and Nxb5 is the engine's best move at +263 for the opposite reason -- the a6 pawn guarding b5 is also the only thing keeping the a-file shut in front of an undefended rook, so axb5 Rxa8 wins it outright with no recapture available. Over 500 games, 506 of 1,376 recommended captures were called trades and 128 of those (25.3%) sat at SEE <= -50, 110 below -100. Below TRADE_FLOOR_CP the word is not used; when every recapture leaves something >=150cp hanging the card names that instead ("costs him the rook on a8 if he takes back", 30 cases, three hand-verified against engine PVs); the rest fall through to the board-verified threat/escape/defends/principle branches (101 keep a true why, 27 lose the why entirely, which is the right way round). The proof uses legally_hanging_pieces, not SEE, because SEE skips king recaptures by design -- a rook landing on f2 in front of a castled king reads as -200 when the engine says +205. # allow-noncentral-caption v153 (2026-09-11): the Socratic surface now reads the diagnosis the analyser already made. Review derived fundamental_violated from two board facts only, so across 8,405 stored Socratic cards 86% used a generic variant and 11 of the 19 variants never fired at all. Of the 7,230 generic cards the move's own cognitive_gap said king_safety on 1,546 (21.4%), missed_tactic on 923 and tactical_oversight on 433 -- all with variants that existed and had never spoken. cognitive_gap now reaches caption_pipeline and maps to a fundamental, but only where the variant says something the gap establishes: opening_knowledge is NOT mapped to development (that variant claims you moved an already-developed piece, which leaving theory does not establish), and endgame_technique, pawn_structure, piece_activity and time_pressure have no honest variant. Board-proved facts still win over the stored label. Making those variants fire exposed a latent leak: their narratives embedded socratic_problem_facts, which were notes written ABOUT the student, so a card read "Your king position got weaker. Student's king is in danger". All seven narratives simply repeated themselves, so the embedding is gone and the evidence strings are written in second person. Recovery plans also stop shipping without a full stop. # allow-noncentral-caption v152 (2026-09-11): selected review moments may reuse immutable candidate evidence to compare the played human-likely idea with one verified stronger line. Read paths stay stored-only; writing is separately gated during analysis/backfill; visible projection is separately gated and fail-closed. v151 (2026-09-11): the Socratic surface finally asks a question. All 19 R18_socratic_user_mistake variants and all 18 R17_coach_move variants shipped their question / hint / hint_for_user fields as empty strings since the rules were written; across 500 stored reviews (29,302 move cards) socratic_coaching fired 2,115 times with a narrative and 0 questions, and coach_move_coaching fired 14,635 times with 0 hints. All 37 fields are authored, the review card renders the question plus a click-to-reveal hint again, and pwc_coaching_lint now fails on an unauthored one. Stored captions must re-render to carry them. Also: the unearned-verdict softener now runs before the board explanation is snapshotted, so the caption and the explanation stop disagreeing ("f3 is playable" vs "f3 is a mistake") and prefixing a personal sentence can no longer make an unearned verdict permanent. # allow-noncentral-caption v150 (2026-09-11): the loose-piece card ("X left your rook on h1 available") now has to be about a mistake. build_legal_material_loss_cause answers a board question -- is something of mine capturable for free after this move -- and the review path was using that answer as a verdict on the move, with no other test at all. Over 500 analysed games it rendered 641 such cards: 63 (9.8%) on moves that cost under 50cp, including a recapture at cp_loss 0 and "Rxf8 ... Kxf8" narrated as a rook left available; 47 (7.3%) in positions already decided by 15 pawns or a mate score (live examples at +9880 and -9990); and 5 whose "punishment" was checkmate, so the card discussed a rook while the player was being mated (Ne2 answered by Qxe2#). Three gates: cost >= 50cp, skip once |eval| >= 1500cp, and abstain when the winning capture is mate so the mate path speaks instead. The cost floor is FLAT, not the rating-band inaccuracy floor -- the bands are a volume control for subtle engine preferences, and using them here silenced a genuine free knight at cp_loss 142 for a 988-rated player. 641 -> 531 cards (82.8% kept). # allow-noncentral-caption v149 (2026-09-09): opponent alternatives can now explain a legally proved capture-recapture-pawn-fork resolution instead of stopping at the misleading immediate capture. v148 (2026-09-09): a piece that is paid for is a trade, not a loss, and an empty reason clause no longer ships broken punctuation. Reading rendered captions (not just counting predicates) turned up "O-O-O runs into Bxd1, losing your rook on d1" where d1 was defended by c1, c3 and f3 and the line was Bxd1 Nxd1 -- rook for bishop, ~170cp, which clears the material floor and so passed v147's gate while the WORDING still claimed an outright loss. walked_into_tactic now says "trading your rook on d1 for a bishop" when material comes back, and one_move_blunder abstains entirely since its text says "lose it for nothing". Also fixed captions shipping as "better was Qg6,." and "instead Kg1 was stronger whereas ..." when best_purpose was empty: the connector was stripped but its punctuation was left stranded. # allow-noncentral-caption v143 (2026-09-07): verdict-softening restricted to the hollow TEMPLATE shape. v142 decided "has a consequence" with a keyword list, so prose captions that explain in other words got softened - "Nxf7 is playable - you moved your knight away from defending e4" calls a move fine and then explains why it isn't. Now bounded by length+sentence count taken from the corpus. v142 (2026-09-07): a mistake verdict must be earned. R12 already refused to assert "X is a mistake" on a user move under 250cp with no why-clause; the fallback paths never got that rule, so 362 captions announced a verdict with nothing behind it (70% at cp 100-199). Same threshold, applied at the composition boundary so every path sees it. Real blunders and captions that name a consequence keep the verdict. v141 (2026-09-06): sentence salvage keeps the board-verified clauses of a caption the claim verifier rejected instead of discarding the whole caption for the hollow comparative floor (qBNJQg3g m16: "Qf6 lets Qxc5 win your bishop on c5" was generated, verified true, and thrown away over a complaint about the NEXT sentence). Plus the narrative truth gate: Truth/Player-Decryption claims are now checked against the game's own trajectory, and a clock loss renders time copy instead of blunder copy. docs/review_truth_layer_scope.md. v140 (2026-09-01): optional pinned Fathom/Syzygy evidence can attach an exact result-change cause for enrolled Quality V2 reviews. Incomplete/unsupported probes abstain; all legacy behavior remains the fallback. v139 (2026-09-01): default-off Personalized Review Quality V2 adds typed, stored-line/legal-board causes, practical game-state framing, cause-derived relationship arrows, strict evidence identity, and a 70-fire/30-negative Caption promotion gate. Existing V1 captions remain unchanged unless the subordinate Quality V2 flag is enabled. v138 (2026-08-31): Stage 4 adds the shared causal/personal explanation contract to Game Review in shadow mode and loads the same evidence-backed player context used by PWC. Visible personalization remains flag-gated. v137 (2026-08-27): hanging-piece facts now use board-mutating legal exchange truth, and free-piece shapes reject x-ray recaptures; invalidates stale false attributions measured in docs/detector_exchange_truth_lock_2026_08_27.md.


# The played-move punishment line shown on "Show me on the board".
...
async def generate_game_decryption_v5(
    ...
    NOTE on game_id: this param is optional in the signature (legacy:
    audit/probe scripts call without it) but if it's None the per-move
    `authored_caption_overrides` lookup is silently SKIPPED — overrides
    never reach the stored caption. Production callers (coach.py route,
    analysis_worker.py initial-analysis path) MUST pass it. A regression
    on this exact gap was caught 2026-06-06 (fb_4899b11157fa).
    """
    if game_id is None:
        # Loud warning so any future caller forgetting game_id surfaces
        # in logs immediately. Authored overrides will be skipped.
        logger.warning(
            "[v5_decryption] game_id=None — authored_caption_overrides "
            "lookup will be silently skipped for every move in this game. "
            "Production callers should pass game_id; only audit/probe "
            "scripts should accept the override-skip."
        )
    try:
        # Parse game
        ...
        # 2026-06-06 — merge opponent mistake/blunder analysis into the
        # lookup so opp moves carry best_move + PV (previously discarded;
        # see stockfish_service Phase 0). Keyed by FEN, opp-to-move
        # positions have different fens than user-to-move positions so
        # there's no collision with the user entries above. Fallback:
        # if not passed in, query the stored analysis by game_id (covers
        # the regen path where the data is already persisted).
        _opp_evals = opponent_move_evaluations
        if _opp_evals is None and game_id:
            try:
                _ga = await db.game_analyses.find_one(
                    {"game_id": game_id},
                    {"_id": 0, "stockfish_analysis.opponent_move_evaluations": 1},
                )
                _opp_evals = (
                    (_ga or {}).get("stockfish_analysis", {})
                    .get("opponent_move_evaluations") or []
                )
            except Exception as _opp_err:
                logger.info(f"[v5] opp eval lookup fetch failed (non-fatal): {_opp_err}")
                _opp_evals = []
        for _oe in (_opp_evals or []):
            _ofen = _oe.get("fen_before", "")
            if _ofen:
        ...
        for idx, move in enumerate(moves):
            ...
            _sev = _compute_severity_for_move(
                ...
                played_move=move,
                prev_move=prev_move,
                # v159: engine truth so the helper can apply the
                # best-equals-played downgrade for BOTH sides.
            ...
            # v159 (2026-09-15): the downgrade itself moved INTO
            # caption_pipeline.compute_severity_for_move so opponent moves
            # get it too (it was `is_user`-only here, which is why
            # "Opponent's Nxh4 is an inaccuracy" shipped on a move the
            # engine itself plays). `severity` already reflects it; the
            # block below now only logs, preserving the existing signal.
            if _sev.played_is_mate:
                logger.info(
                    f"[SEVERITY-SANITY] move {move_san} is checkmate; "
                    f"severity forced to good"
                )
            if severity in ("inaccuracy", "mistake", "blunder"):
            ...
            if CAPTION_V5_PIPELINE_ENABLED and _build_move_teaching_decision is not None:
                try:
                    ...
                    caption_payload = {
                        ...
                        # Belongs to board_before + the best move, NOT to the
                        # position this card renders. Shipped separately so it
                        # can only be drawn where it is true.
                        "best_move_arrows": _decision.visual.best_move_arrows,
                        "best_move_arrows_fen": _decision.visual.best_move_arrows_fen,
                    }
                    ...
                    if _decision.coach_line_moves:
                        ...
                        _coach_line_length_hint_for_iter = (
                            _decision.coach_line_length_hint
                            or len(_coach_line_moves_for_iter)
                        )
                    elif is_user and (cp_loss or 0) >= PUNISHMENT_LINE_MIN_CP:
                        # The punishment line for OUR OWN mistakes.
                        #
                        # build_move_teaching_decision only builds a coach line
                        # for OPPONENT mistakes (its gate is `not is_user AND
                        # opp_cp_loss >= 30`), so a user mistake reached the
                        # card with coach_line_moves=None and
                        # coach_line_length_hint=None -- and the review UI has
                        # three ways to find a line, all of which then fail:
                        # no trap, no coach line, and the pv_after_best branch
                        # needs the hint it never got. So the board sat still
                        # on exactly the moves a player most wants explained.
                        #
                        # Every existing branch also animates pv_after_BEST --
                        # the line where you play well. What answers "why was
                        # my move bad" is pv_after_PLAYED: what the opponent
                        # does to you. Nothing read it.
                        #
                        # Found on eb189840 move 8 (Nf3, -191cp), where the
                        # loss is seven forced captures netting a pawn --
                        # Nxb5 Bxf6 Nxc3 Bxe7 Nxd1 Bxd8 Nxb2. No single piece
                        # hangs (SEE is 0 on every one), so the arrow builders
                        # correctly drew nothing and no sentence could carry
                        # it either. Played out on the board it is obvious.
                        _pv_played_line = [
                            str(x).strip()
                            for x in (eval_data.get("pv_after_played") or [])
                            if str(x or "").strip()
                        ...
                        if _pv_played_line:
                            ...
                            _coach_line_moves_for_iter = (
                                [move_san] + _pv_played_line
                            )
                            _coach_line_length_hint_for_iter = len(
                                _coach_line_moves_for_iter
                            )
            ...
            try:
                ...
                if _final_cap:
                    ...
                    _vfacts = {
                        ...
                        # Let the verifier read the EVIDENCE for who is mating
                        # instead of inferring it from the prose. Without this
                        # it falls back to "a user move that mentions mate must
                        # mean our best move mates" and softens away every
                        # caption about a mate the opponent delivers.
                        "mate_threat_evidence": (caption_facts or {}).get(
            ...
            move_output = {
                ...
                "best_move_arrows": caption_payload.get("best_move_arrows") or [],
                "best_move_arrows_fen": caption_payload.get("best_move_arrows_fen") or "",
