...
async def generate_game_decryption_v5(
    ...
    try:
        ...
        for idx, move in enumerate(moves):
            ...
            # v58 (2026-05-22): reconcile best_move with pv_after_best[0] when
            # they disagree. The stored `best_move` can be stale (computed at
            # different depth than the multipv PV); when this happens captions
            # name a move the engine isn't actually planning to play first.
            # Mohit caught this on row #003: stored best_move=Bxa6 but the
            # depth-15 PV starts with Nxe5 (the real engine plan) — caption
            # said "wins the knight on a6" but the actual lesson is "wins the
            # e5 pawn". Same root cause as row #034 (b6/Na5). Trust the PV.
            if pv_after_best and best_move and pv_after_best[0] != best_move:
                best_move = pv_after_best[0]
            ...
            if CAPTION_V5_PIPELINE_ENABLED and _extract_caption_facts and _render_caption_dict:
                try:
                    ...
                    if is_user and (cp_loss or 0) >= 50:
                        try:
