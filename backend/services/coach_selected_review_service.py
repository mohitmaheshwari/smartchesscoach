"""One evidence-gated selector and lifecycle for player Game Review.
...
def _focus_matches(focus_key: str, event: Mapping[str, Any]) -> bool:
    ...
    return focus.issubset(observed) or bool(focus & observed)
...
def _event_preview(event: Mapping[str, Any], role: str) -> Optional[Dict[str, Any]]:
    ...
    labels = {
        ...
        "strong_decision": "What you understood well",
    }
    return {
...
def candidate_from_projection(
    *,
    ...
    for chapter in plan.get("chapters") or []:
        ...
        preview = (
            _event_preview(event, str(chapter.get("role") or ""))
...
def _projection_env(env: Optional[Mapping[str, str]]) -> Dict[str, str]:
    ...
    return {
        FEATURE_FLAG: "true",
        QUALITY_V2_FEATURE_FLAG: "true" if personalized_review_quality_v2_enabled(source) else "false",
    }
...
async def _load_candidates(
    ...
    *,
    ...
    projection = {
        "_id": 0, "game_id": 1, "opponent": 1, "opponent_name": 1,
        "result": 1, "user_color": 1, "opening": 1, "opening_name": 1,
        "platform": 1, "date_played": 1, "imported_at": 1,
        "analyzed_at": 1, "created_at": 1,
    }
    games = await db.games.find(query, projection).to_list(None)
    by_id = {str(game["game_id"]): game for game in games if game.get("game_id")}
    if not by_id:
        return []
    analyses = await db.game_analyses.find(
        {"game_id": {"$in": list(by_id)}},
        {"_id": 0, "game_id": 1, "decryption_v5_data": 1, "game_teaching_plan": 1},
    ).to_list(None)
    result = []
    for analysis in analyses:
        ...
        if game is None or not isinstance(raw_plan, Mapping) or not isinstance(moves, list):
            continue
        projected = maybe_attach_phase5_review_fields(
            {"decryption_data": []},
            stored_moves=tuple(moves),
            stored_plan=raw_plan,
            env=_projection_env(env),
        )
        candidate = candidate_from_projection(
            game=game, raw_plan=raw_plan, projection=projected, focus_key=focus_key
        )
        if candidate:
...
async def _focus_key(db, user_id: str) -> str:
    focus = await db.user_active_focus.find_one(
        ...
        {"_id": 0, "topic_key": 1, "focus_kind": 1, "detector_quality_id": 1},
    )
    if not isinstance(focus, Mapping):
        return ""
    return str(focus.get("topic_key") or focus.get("focus_kind") or focus.get("detector_quality_id") or "").strip()
...
async def _candidate_for_document(
    db, user_id: str, document: Mapping[str, Any], *, env: Optional[Mapping[str, str]]
) -> Optional[Dict[str, Any]]:
    candidates = await _load_candidates(
        db, user_id, focus_key=str(document.get("focus_key") or ""),
        excluded_game_ids=(), env=env,
    )
    for item in candidates:
        if (
            item["game_id"] == document.get("game_id")
            and item["plan_id"] == document.get("plan_id")
            and item["plan_fingerprint"] == str(document.get("plan_fingerprint") or "")
        ):
            return item
    return None


def _response(document: Mapping[str, Any], candidate: Mapping[str, Any]) -> Dict[str, Any]:
    return {
...
async def get_recommendation(
    ...
    *,
    ...
    access = await get_complete_coaching_access(db, user_id, user_doc=user_doc, env=env)
    if not access.enabled:
        return {
            "schema_version": SCHEMA_VERSION, "enabled": False,
            "paused": access.paused, "status": "disabled",
            "reason": access.reason, "prescription": None,
        }
    collection = db[COLLECTION]
    active = await collection.find_one({"user_id": user_id, "is_active": True}, {"_id": 0})
    if active:
        candidate = await _candidate_for_document(db, user_id, active, env=env)
        if candidate is not None:
            return _response(active, candidate)
        now = _now()
        await collection.update_one(
            {"prescription_id": active.get("prescription_id"), "user_id": user_id, "is_active": True},
            {"$set": {
                "state": "superseded", "is_active": False,
    ...
    candidates = await _load_candidates(
        db, user_id, focus_key=focus_key,
        excluded_game_ids=await _terminal_game_ids(db, user_id), env=env,
    )
    if not candidates:
        return {
            "schema_version": SCHEMA_VERSION, "enabled": True, "paused": False,
            "status": "empty", "prescription": None,
            "empty": {
                "headline": "I have not found the right game to teach from yet.",
                "body": "Your analyzed games stay below. I will choose one when its lesson is fully verified.",
                "primary_action": {"label": "Play with Coach", "href": "/play-with-coach"},
                "secondary_action": {"label": "Import recent games", "href": "/import"},
            },
        }
    chosen = candidates[0]
    seed = f"{user_id}:{chosen['game_id']}:{chosen['plan_id']}:{chosen['plan_fingerprint']}:{SELECTOR_VERSION}"
    now = _now()
    document = {
        "prescription_id": f"grp_{hashlib.sha256(seed.encode()).hexdigest()[:24]}",
        "user_id": user_id, "game_id": chosen["game_id"],
        "state": "recommended", "is_active": True,
        "selector_version": SELECTOR_VERSION,
        "plan_id": chosen["plan_id"],
        "plan_fingerprint": chosen["plan_fingerprint"],
        "focus_key": focus_key, "focus_match": bool(chosen["focus_match"]),
    ...
    try:
        ...
        document = await collection.find_one({"user_id": user_id, "is_active": True}, {"_id": 0})
        if not document:
            raise
        chosen = await _candidate_for_document(db, user_id, document, env=env)
        if chosen is None:
            raise ReviewPrescriptionError("active prescription is no longer valid")
    return _response(document, chosen)


async def _owned_active(db, user_id: str, prescription_id: str) -> Dict[str, Any]:
    document = await db[COLLECTION].find_one(
        {"prescription_id": prescription_id, "user_id": user_id, "is_active": True},
        {"_id": 0},
    )
    if not document:
        raise ReviewPrescriptionError("active review recommendation not found")
    return document


async def start_prescription(
    db, user_id: str, prescription_id: str, *, env: Optional[Mapping[str, str]] = None
) -> Dict[str, Any]:
    document = await _owned_active(db, user_id, prescription_id)
    if document.get("state") not in ACTIVE_STATES:
        raise ReviewPrescriptionError("review cannot be started from this state")
    now = _now()
...
async def save_progress(db, user_id: str, prescription_id: str, move_index: int) -> Dict[str, Any]:
    if isinstance(move_index, bool) or not isinstance(move_index, int) or move_index < -1:
        raise ReviewPrescriptionError("move_index must be an integer of -1 or greater")
    document = await _owned_active(db, user_id, prescription_id)
    if document.get("state") != "started":
        raise ReviewPrescriptionError("start the review before saving progress")
    now = _now()
    await db[COLLECTION].update_one(
        {"prescription_id": prescription_id, "user_id": user_id, "is_active": True},
        {"$set": {
            "resume.move_index": move_index, "resume.updated_at": now, "updated_at": now,
        }},
    )
    return {"saved": True, "move_index": move_index, "updated_at": now.isoformat()}
...
async def dismiss_prescription(
    ...
    await db[COLLECTION].update_one(
        {"prescription_id": prescription_id, "user_id": user_id, "is_active": True},
        {"$set": {"state": "dismissed", "is_active": False, "dismissed_at": now, "updated_at": now}},
    )
    return await get_recommendation(db, user_id, env=env)


async def complete_prescription(
    db, user_id: str, prescription_id: str, *, game_id: str,
    env: Optional[Mapping[str, str]] = None,
) -> Dict[str, Any]:
    document = await _owned_active(db, user_id, prescription_id)
    if document.get("game_id") != game_id:
        raise ReviewPrescriptionError("prescription does not belong to this game")
    if document.get("state") != "started":
        raise ReviewPrescriptionError("review must be started before completion")
    now = _now()
    await db[COLLECTION].update_one(
        {"prescription_id": prescription_id, "user_id": user_id, "is_active": True},
        {"$set": {"state": "completed", "is_active": False, "completed_at": now, "updated_at": now}},
    )
    return await get_recommendation(db, user_id, env=env)


__all__ = [
    "COLLECTION", "SCHEMA_VERSION", "SELECTOR_VERSION",
    "ReviewPrescriptionError", "candidate_from_projection",
    "complete_prescription", "dismiss_prescription",
    "ensure_review_prescription_indexes", "get_recommendation",
    "public_prescription", "rank_candidates", "save_progress",
    "start_prescription",
]
