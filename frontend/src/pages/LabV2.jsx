...
const LabV2 = ({ user }) => {
  ...
  // Mohit 2026-06-01: `?move=N` URL was a no-op because LabV2 (the
  // component routed at /game/:gameId — earlier fix landed on the
  // legacy GameAnalysis.jsx by mistake) never read searchParams.
  // Read the param, then in an effect after game data loads we
  // convert the 1-indexed full move number to a 0-indexed ply and
  ...
  // Mohit 2026-06-01: jump to the URL-specified move once PGN parses.
  //
  // URL emitters across the app (EvidenceModal, MasteryPanel, mastery
  // cards, Lab links generated by the game-mirror coach lines) all
  // use the convention ?move=N where N is the 1-indexed FULL move
  // number — same number the player sees in the move list and the
  // same value stored on coach_memory evidence entries.
  //
  // The board uses a 0-indexed PLY index (white moves at even
  // indices, black at odd). Convert by user color so we land on the
  // position AFTER the user's Nth move (which is the move the URL
  // emitter is pointing at — every emitter is about a user move).
  //
  // We fire ONCE after moves have parsed, and only when the value is
  // in range. Otherwise the board would jump again whenever the user
  // navigated manually.
  useEffect(() => {
