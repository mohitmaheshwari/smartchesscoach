...
export default function AdminPositionalReasons() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const draft = data.generated_candidate;
      if (draft) {
        setForm({
          ...EMPTY_FORM,
          disposition: draft.disposition || "eligible_positional",
          concept_label: draft.concept_label || "",
          canonical_concept_id: draft.canonical_concept_id || "",
          better_move_fact: draft.better_move_fact || "",
          played_move_fact: draft.played_move_fact || "",
          contrast: draft.contrast || "",
          transferable_lesson: draft.transferable_lesson || "",
          notes: "",
        });
      }
    } catch (fetchError) {
          ...
          {progress && (
            <div className="max-w-xl text-right text-xs text-muted-foreground">
              ...
              {progress.generated > 0 && (
                <div>
                  {progress.verified_generated_captions || 0} checked caption drafts generated
                </div>
              )}
        ...
        {loading ? (
          ...
          <div className="grid gap-6 lg:grid-cols-[380px_minmax(0,1fr)]">
            ...
            <div className="space-y-5">
              {position.generated_candidate && (
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4 text-sm">
                  <div className="flex items-center gap-2 font-medium">
                    <ShieldCheck className="h-4 w-4 text-emerald-500" />
                    Board-checked draft ready for review
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {position.generated_candidate.disposition === "eligible_positional"
                      ? "The four teaching fields were generated from legal board geometry and verified continuations. Review and edit before saving."
                      : `Suggested disposition: ${
                          DISPOSITIONS.find(
                            (item) => item.id === position.generated_candidate.disposition
                          )?.label || position.generated_candidate.disposition
