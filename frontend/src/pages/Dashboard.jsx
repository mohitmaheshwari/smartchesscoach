...
import { Loader2, Import, ChevronRight, Check, TrendingDown, TrendingUp, Minus, Target, Sparkles } from "lucide-react";
      ...
      <div className="max-w-3xl mx-auto py-8 px-4" data-testid="lab-page">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="flex items-center gap-4 mb-3">
                ...
                <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-md">last {verdict.total} games</span>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-8">
            ...
            >
              ...
              <div className="relative p-5">
                ...
                {pick.lesson_label && (
                  <span className="inline-block text-[10px] font-bold uppercase tracking-[0.12em] px-2 py-0.5 rounded-md bg-primary/10 text-primary border border-primary/20 mb-3">
                    {pick.lesson_label}
                  </span>
                )}
                ...
                {pickPattern && (
                  <button
                    className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:text-primary/80 transition-colors bg-primary/5 px-3 py-1.5 rounded-lg"
        ...
        {unreviewedGames.length > (pick ? 1 : 0) && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            <p className="text-[10px] tracking-[0.15em] uppercase mb-3 font-bold text-muted-foreground/70">
        ...
        {reviewedGames.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="mt-8">
            <p className="text-[10px] tracking-[0.15em] uppercase mb-3 font-bold text-muted-foreground/40">
              Reviewed ({reviewedGames.length})
            </p>
            <div className="bg-card/50 border border-border/50 rounded-xl divide-y divide-border/50 overflow-hidden">
              {reviewedGames.map((game) => (
                <div
                  key={game.game_id}
                  className="flex items-center gap-3 px-4 py-3 cursor-pointer transition-all hover:bg-muted/20 opacity-50 hover:opacity-75"
        ...
        {games.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20" data-testid="lab-empty-state">
            ...
            <h2 className="text-xl font-heading font-semibold text-foreground mb-2">Your lab is empty</h2>
...
const GameRow = ({ game, navigate, markReviewed }) => (
  <div
    className="flex items-start gap-3.5 px-4 py-4 cursor-pointer transition-all hover:bg-muted/20 group"
    onClick={() => navigate(`/game/${game.game_id}`)}
    data-testid={`game-row-${game.game_id}`}
  >
    <div className="mt-0.5 flex-shrink-0"><AccuracyDot accuracy={game.accuracy} /></div>
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2 mb-0.5">
        <span className="text-sm font-medium text-foreground">vs {game.opponent}</span>
        <ResultBadge result={game.result} small />
        {game.opening && (<><span className="text-muted-foreground/20">&middot;</span><span className="text-xs text-muted-foreground truncate">{game.opening}</span></>)}
      </div>
      {(game.behavior || game.lesson) ? (
        <div className="mt-1">
          {game.lesson_label && (<span className="text-[9px] font-bold uppercase tracking-[0.1em] text-primary mr-2">{game.lesson_label}</span>)}
    ...
    <button
      ...
      title="Mark as reviewed" data-testid={`mark-reviewed-${game.game_id}`}
    >
...
const AccuracyDot = ({ accuracy }) => {
  ...
  const color = accuracy >= 85 ? 'bg-emerald-500' : accuracy >= 70 ? 'bg-emerald-400' : accuracy >= 55 ? 'bg-amber-400' : 'bg-red-400';
  return <div className={`w-2.5 h-2.5 rounded-full ${color}`} title={`${accuracy.toFixed(0)}% accuracy`} />;
};
