...
a board-verifier. **Quality comes from Codex (offline, once); truth comes from the verifier
...
1. **Pull the gold for the situation.** `db.gold_captions` filtered by `cognitive_gap` (+ optionally
   ...
   --per-gap N` (engine-verified Codex gold; ~40-60 examples per situation is the working number).
...
3. **Distill ONE template** (Codex, offline, ONCE per situation). Prompt: "here are N (FACTS, GOLD)
   examples; write one str.format template using ONLY these slots {…}; max 2 sentences; end with one
   fixed universal principle." Codex returns `{"template": "..."}`. This is the only Codex call that
...
- Don't run runtime Codex. Distillation is offline; rendering is template-fill + verify.
...
- Per-situation cost (measured): ~35-60 gold (mostly already collected) + ~2 hrs slot work. No big
  Codex spend. Multiply by ~40-60 engine-decidable situations; positional ones abstain.
