...
This promotion lets ChessGuru explain a missed mating opportunity after a puzzle attempt when the stored engine evidence identifies the best move and an independent legal replay reaches checkmate for the player. Mate in one says what the move does. Longer cases show the complete stored line and explicitly warn that the opponent may defend differently. It does not call the idea forced unless the stored evidence and promotion audit independently support that stronger statement, and it does not diagnose a recurring weakness.
...
One line: Kg1 Kh8 Ra8#. At the end the king on h8 has no legal reply. The opponent can defend differently, so check their other tries too.
Before playing a quiet move, examine every check, capture and direct threat first.
```

If the available facts prove only one legal stored continuation and not every defence, the caption labels it `One line`, shows every move, and says the opponent can defend differently. It does not say `forced`, `unavoidable`, or `mate in N`.
...
- Re-admit only stored rows already carrying `tactic:forced_mate_exact`, recovering
  non-persisted consequence evidence from the source game analysis. Stage the
  entire targeted batch and permit no write unless every rebuilt caption fact
  matches independent legal replay and every ineligible row abstains.
...
- Reanalysis of existing games, untargeted puzzle-pool backfills, deployment, or visible progress changes.
...
- Every stored caption fact matches the independent verifier: full replayed line, final mating move, mating piece and square, king square, and zero legal replies at the terminal position. The rendered caption shows the mate-in-one SAN or complete longer line plus the terminal king fact without restating redundant piece/square text.
...
## 8. Scope amendment — targeted re-admission

On 2026-09-05 Mohit explicitly authorized the stored forced-mate re-admission.
The authorization is limited to rows whose existing
`verified_admission.quality_id` is `tactic:forced_mate_exact`. The migration must
recover source analysis rather than infer missing consequence, validate the
actual rendered caption per FEN, abort before all writes on any violation, and
perform a post-write version/grade readback. No other quality family or player
state is included.
