...
| AR-QA-003 | Caption source guard is invoked with `|| true`, so a detected violation cannot fail CI. | **VERIFIED CURRENT · CODEX** | Guard returns meaningful status and blocks the release when its governed contract fails; intentional exceptions are explicit and tested. |
...
| ID | Finding and current evidence | State / owner | Closure proof |
|---|---|---|---|
| AR-QA-004 | `package.json` declared Yarn and production built with Yarn, but the repository had no `yarn.lock`; its tracked npm lock both conflicted with React 19 through unused `react-day-picker@8.10.1` and was not consumed by production. Local npm and production Yarn therefore resolved different trees. | **SOURCE COMPLETE · CODEX · DEPLOYED PROOF PENDING** | The unused incompatible dependency is removed; one Yarn lock is versioned as the sole resolution authority; every production build uses `--frozen-lockfile`; an empty-directory frozen install, all 30 frontend suites / 162 tests, and the production build pass. Independent diff review and deployed build proof remain. |
| AR-QA-005 | Phase 8's mandatory deployment verifier sends its ordinary web session as `Authorization: Bearer`, but Phase 1 correctly reserves bearer transport for `is_mobile=true` sessions. Combining the two source-complete phases would make every strict deployment fail authentication or tempt an unsafe bearer rollback. | **VERIFIED INTEGRATION DEFECT · CODEX** | All verifier HTTP calls exercise the `session_token` cookie boundary; no Authorization header is sent; the preferred secret/CLI name says session cookie; the former secret/CLI input is accepted only as a temporary value alias; tests prove cookie-only requests and the strict deploy script forwards the cookie credential. |
...
| AR-AUTH-002 | Google OAuth `state` is the unsigned string `platform___redirect_to`; the login endpoint accepts the redirect from request input and interpolates the state into the provider URL manually. The callback does not bind the return to the browser that initiated it. | **VERIFIED CURRENT · CODEX** | Cryptographically authenticated, expiring state is bound to an HttpOnly initiator cookie; redirect destinations are local paths; tampered, expired, missing-cookie and mismatched state fail before provider exchange; web and mobile return paths pass. |
| AR-AUTH-003 | `POST /api/auth/demo-login` is reachable without checking `DEV_MODE`, accepts an arbitrary email, and reuses an existing account's `user_id` when the email matches. Before the mobile-only bearer boundary in Phase 1, this was an account-impersonation path; relying only on that downstream rejection is not sufficient closure. | **VERIFIED CURRENT · CODEX** | Production returns `403` before database access; dev-only demo identities use a non-email hashed `demo_` id and cannot alias real accounts; any returned credential is explicitly mobile-only; security tests lock the gate. |
