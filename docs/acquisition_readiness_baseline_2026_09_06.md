...
| AR-QA-006 | The ordinary production frontend build succeeds with warnings, but the same build under `CI=true` initially failed on 71 `react-hooks/exhaustive-deps` findings across core Review, Play-with-Coach, board, opening and navigation surfaces. The first characterized navigation slice reduced the measured inventory to 69; the remaining debt may include stale-closure behavior, not just style noise. | **REMEDIATION IN PROGRESS · CODEX · 69 REMAIN** | Review each finding behaviorally; fix or narrowly justify it; make the CI production build pass without globally suppressing the rule; rerun all frontend tests and interactive golden journeys. |
