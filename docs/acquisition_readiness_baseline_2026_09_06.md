...
| ID | Finding and current evidence | State / owner | Closure proof |
|---|---|---|---|
| AR-QA-004 | A clean `npm ci` on the canonical lockfile fails dependency resolution because `react-day-picker@8.10.1` declares React 16–18 while the application declares React 19. `npm ci --legacy-peer-deps` installs and the production build succeeds, but the override is an undocumented supply-chain requirement. | **VERIFIED CURRENT · CODEX** | Dependency versions are made compatible or the exception is explicitly pinned, justified, tested and used identically in local/CI/deploy clean installs. |
