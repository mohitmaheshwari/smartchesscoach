...
**Never measure the success of a capability that has not yet been
intentionally designed to exist, added 2026-08-06.** A recall test run
today would fail — not because recall isn't real, but because no
caption currently contains anything designed to be recalled. That
result would carry zero information; you can't fail to remember
something that was never stated. This generalizes past recall
specifically: before measuring whether a capability works, confirm the
capability has actually been built with intent, not just that some
proxy of it exists incidentally. Otherwise the number just confirms the
obvious and gets mistaken for a real finding.
