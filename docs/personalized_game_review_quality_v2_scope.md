...
**Status:** PROPOSED — awaiting Mohit signoff  
**Date:** 2026-09-01  
...
- Hand-editing the legacy R12 cascade to force one example to read correctly.
...
- Automatically promoting missed mate, threat awareness, opening, trap, endgame or positional detectors without their evidence packets passing the existing quality gates.
...
- **Question:** Which practical-state ranking formula should choose between a huge mate-score loss that stays winning and a smaller move that changes the game state?  
  **Why unresolved:** Raw centipawn loss, win-probability change and state transition can rank the same moments differently.  
  **Unblocking step:** Bake off the existing planner formula against practical-state alternatives on stratified wins, losses and missed-mate games, then lock the winner from blinded coach judgments.
...
- Mohit explicitly signs off this complete Quality V2 scope document.
