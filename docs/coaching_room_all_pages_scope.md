...
- **Question:** Which home/library responses already expose a safe, relevant preview position? **Why unresolved:** `CurriculumPrimary` currently consumes title/reason/evidence/destination, not a universally available preview contract. **Unblocking step:** inspect the existing response projections before coding the slice; use the compact non-board variant where unavailable. Any new backend contract needs separate approval.
