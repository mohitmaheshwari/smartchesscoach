...
5. **Phase 3 — chess-intelligence breadth.** Begin with **Phase 3A: Hidden Opportunities Causal Coaching**, defined in `docs/hidden_opportunities_causal_coaching_scope.md`. Phase 3A must compare the played and better branches, prove setup → constraint → payoff, promote mechanism families independently, rank only proved coaching moments, and integrate them into the canonical Game Review interaction before broader detector-count expansion. After that foundation passes, select further detector families from measured prevalence, distinctiveness, proofability, opportunity coverage, and lesson readiness. Build/polish proof families, independent gold, adversarial packets, Caption promotion, then Plan/Mastery promotion where earned.
