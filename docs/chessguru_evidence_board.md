...
| **Player Identity** (`player_identities` — style_profile, behavioral_profile, learning_velocity) | Personalization — "does the coach know me" | Yes — 63/114 users, read unconditionally into game review and Home | Medium — real differentiated style data, but narrow effect (threshold-gating only, not sentence content) | Partial — `style_profile` genuinely computed; `post_blunder_accuracy`, `recovery_capability`, `plays_worse_after_loss`, `learning_velocity.overall_improvement_rate` confirmed stuck at identical hardcoded defaults for every user | **Finish wiring** the dead-default fields or delete them from the model (Bucket B, item 7) |
...
| **Teaching Recall** (`user_teaching_memory`) | "Have I explained this to you before" cross-session recall | Partial — read path live, write path orphaned since 2026-05-18 | Low and degrading — confidently serves 2.5+ months of stale memory | None | **Bucket A**: fix the writer (cheap — restore one caller) or retire the read path; do not leave it silently decaying |
