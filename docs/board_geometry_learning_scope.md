...
- **Question:** What live-game prompt quota and cooldown preserve learning without making PWC feel interrupted?  
  **Why unresolved:** The existing always-on scope chose a visual cap from judgment, while the recent PWC audit showed that noise and late content materially damage the experience.  
  **Unblocking step:** Replay verified opportunities across the existing PWC corpus, measure opportunities per game by rating and family, then run `/lock-via-data` before choosing the quota.

- **Question:** How many reviewed teaching, counterexample, unseen assessment, and delayed-recall positions exist for each of the four modules?  
  **Why unresolved:** The signed fork scope measured a candidate pool, but the other three modules do not yet have frozen, independently reviewed content sets.  
  **Unblocking step:** Inventory existing diagnostic, motif, community, and personal positions; produce a family-by-role coverage table and author only the proven shortfall.

- **Question:** Which current detector implementation supplies each canonical geometry primitive after consolidation?  
  **Why unresolved:** `caption_facts.py`, `shape_detectors.py`, and `geometry_plans.py` overlap today, and the last two cannot independently become a second product authority.  
  **Unblocking step:** Write the architecture ownership map, compare outputs on the same corpus, keep the caption-facts contracts required by the signed learning architecture, and extract only missing future-square primitives into that shared path.

- **Question:** When may the real-user geometry experiment run relative to the currently active product-learning experiment?  
  **Why unresolved:** The repository permits one active product-learning experiment unless orthogonality is proven.  
  **Unblocking step:** Keep implementation and internal validation default-off; before assigning a real-user cohort, close the active experiment or record an explicit orthogonality ruling.
